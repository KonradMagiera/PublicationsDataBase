...
def logout():
    ...
    response = requests.post(api + "logout", headers=headers)
    if(response.status_code == 200):
        return "", ""
    else:
        print("\nerror occured\n")
...
def publications():
    ...
    response = requests.get(api + "publications", headers=headers)
    pubs = list()
    data = response.json()
...
def add_publication():
    ...
    while(True):
        ...
        if(len(date) == 10 or date == ""):
            break
    headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
    response = requests.post(api + "publications", json={"title": title, "author": author, "publisher": publisher, "date": date}, headers=headers)
    data = response.json()
    id = -1
    if("id" in data):
        id = data["id"]
...
def edit_publication(id):
    ...
    while(True):
        ...
        if(len(date_tmp) == 10):
            ...
            break
    headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
    requests.put(api + "publications/" + str(id), json={"id": str(id), "title": title, "author": author, "publisher": publisher, "date":date}, headers=headers)
    
def delete_publication(id):
    headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
    requests.delete(api + "publications/" + str(id), headers=headers)
...
def get_pub_data(id):
    headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
    response = requests.get(api + "publications/" + str(id), headers=headers)
    data = response.json()
...
def get_pub_files(id):
    headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
    response = requests.get(api + "publications/" + str(id) + "/files", headers=headers)
    files = response.json()
...
def add_file(id):
    ...
    if(filename != ""):
            headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
            response = requests.post(api + "publications/" + str(id) + "/files", files=files, headers=headers)
...
def delete_file(pid, fid):
    headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
    requests.delete(api + "publications/" + str(pid) + "/files/" + str(fid), headers=headers)

def download_file(pid, fid):
    headers= {"Authorization": create_jwt(PUBLICATIONS_ACCESS)}
    response = requests.get(api + "publications/" + str(pid) + "/files/" + str(fid), headers=headers)
    if(response.status_code != 200):
        return False
